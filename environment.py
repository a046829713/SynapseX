...
class Env(gym.Env):
    def __init__(self, prices, state, random_ofs_on_reset):
        ...
        # 假設離散動作有3個：0=清倉/空手, 1=買入, 2=賣出(如果做空)
        # 如果只做多，可以是：0=清倉/空手, 1=持有多倉
        N_DISCRETE_ACTIONS = 2 # 假設只做多 (0: 清倉, 1: 買入/持有)
...
def calculate_reward(self):
    # 1. 基礎獎勵：對數回報
    log_return = np.log(self.portfolio_value_at_t / self.portfolio_value_at_t_minus_1)
    
    # 2. 交易成本懲罰 (這是資產組合價值變化中隱含的，但明確計算更好)
    # 事實上，如果你的 portfolio_value 計算已經包含了手續費，這裡就不需要重複扣除
    # 但我們可以把它作為一個獨立的懲罰項來放大它的影響
    FEE_RATE = 0.001
    position_change = abs(self.current_position - self.previous_position)
    transaction_penalty = position_change * FEE_RATE # 作為懲罰，所以是負值

    # 3. 風險懲罰：基於回報的波動性
    # 如果當前回報遠低於平均回報（一個負向的意外），給予懲罰
    # 這是 Sortino Ratio (索提諾比率) 的思想，只懲罰下行風險
    drawdown_penalty = 0.0
    if len(self.returns_history) > 1:
        mean_return = np.mean(self.returns_history)
        step_return = (self.portfolio_value_at_t / self.portfolio_value_at_t_minus_1) - 1
        # 如果回報為負，懲罰其與均值的差距的平方，放大懲罰效果
        if step_return < 0:
            drawdown_penalty = (step_return - mean_return)**2 # 懲罰項，所以是負值
    
    # 4. 空手獎勵/持倉懲罰
    idle_incentive = 0.0
    if self.current_position == 0:
        idle_incentive = 1e-5 # 微小的正獎勵
    
    # --- 組合最終獎勵 ---
    # w1, w2, w3 是超參數，需要調整來平衡各項的重要性
    w1, w2, w3, w4 = 1.0, 1.0, 0.5, 1.0 
    
    final_reward = (w1 * log_return -
                    w2 * transaction_penalty -
                    w3 * drawdown_penalty +
                    w4 * idle_incentive)
...
class Reward():
    def __init__(self):
        self.log_returnWeight = 0

    def log_return(self):
        pass

    def transaction_penalty(self):
        pass

    def drawdown_penalty(self):
        pass
