...
def CreateNetworks(
    ...
    with tf.variable_scope('mu'):
        mu = action_max * ANN(s, list(hidden_sizes) +
                              [num_actions], hidden_activation, output_activation)
    with tf.variable_scope('q'):
        input_ = tf.concat([s, a], axis=-1)  # (state, action)
        q = tf.squeeze(ANN(input_, list(hidden_sizes) +
                       [1], hidden_activation, None), axis=1)
    with tf.variable_scope('q', reuse=True):
        ...
        q_mu = tf.squeeze(ANN(input_, list(hidden_sizes) +
                          [1], hidden_activation, None), axis=1)
    return mu, q, q_mu
...
def ddpg(
    ...
    with tf.variable_scope('target'):
        ...
        _, _, q_mu_targ = CreateNetworks(
            X2, A, num_actions, action_max, **ac_kwargs)

    # Experience replay memory
    replay_buffer = ReplayBuffer(
        obs_dim=num_states, act_dim=num_actions, size=replay_size)
    ...
    def test_agent(num_episodes=5):
        ...
        for j in range(num_episodes):
            ...
            while not (d or (episode_length == max_episode_length)):
                ...
                episode_length += 1
                n_steps += 1
            print('test return:', episode_return,
    ...
    for i_episode in range(num_train_episodes):
        ...
        for _ in range(episode_length):
            ...
            mul, _, _ = sess.run(
                [mu_loss, mu_train_op, target_update], feed_dict)
    ...
    np.savez('ddpg_results.npz', train=returns, test=test_returns,
             q_losses=q_losses, mu_losses=mu_losses)
