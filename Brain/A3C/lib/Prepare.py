...
class RL_prepare(ABC):
    def __init__(self):
        ...
        self.prepare_keyword()
        self.prepare_device()
        self.prepare_symbols()
        self.prepare_hyperparameters()
        self.prepare_envs()
        self.prepare_model()
        # self.prepare_targer_net()
        # self.prepare_agent()
    ...
    def prepare_device(self):
        ...
        show_setting("A3C:", "distribution train only can be cpu right now ")
    ...
    def prepare_envs(self):
        ...
        if self.keyword == 'Transformer':
            ...
            data = OriginalDataFrature().get_train_net_work_data_by_path([
                self.symbols[0]])
        ...
        for symbol in self.symbols:
            data = OriginalDataFrature(
            ).get_train_net_work_data_by_path([symbol])
            # 製作環境
            self.train_envs.append(
                Env(prices=data, state=state, random_ofs_on_reset=True))
    ...
    def prepare_optimizer(self):
        # 定義特殊層的學習率
        param_groups = [
            {'params': self.model.dean.mean_layer.parameters(), 'lr': 0.0001 *
             self.model.dean.mean_lr},
            {'params': self.model.dean.scaling_layer.parameters(
            ), 'lr': 0.0001 * self.model.dean.scale_lr},
            {'params': self.model.dean.gating_layer.parameters(), 'lr': 0.0001 *
             self.model.dean.gate_lr},
        ]
