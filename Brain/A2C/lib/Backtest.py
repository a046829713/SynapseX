...
class RL_evaluate():
    ...
    def load_model(self, model_path: str):
        ...
        model = TransformerModel(
            ...
            d_hid=2048,
            nlayers=2,
            n_actions=self.evaluate_env.action_space.n,
            hidden_size=64,
            dropout=0.1,
            batch_first=True,
    ...
    def test(self):
        ...
        record_orders = []
        state = torch.tensor(self.evaluate_env.reset(), dtype=torch.float32).to(self.device).unsqueeze(0)
        
        with torch.no_grad():
            while not done:
                policy, _ = self.agent(state)
                logits = policy.view(-1)
                # 取最大值的索引作為行動
                action = logits.argmax().item()
                record_orders.append(action)                
                next_state, reward, done, _ = self.evaluate_env.step(action)            
                state = torch.tensor(next_state, dtype=torch.float32).to(self.device).unsqueeze(0)
