...
class RL_prepare(ABC):
    ...
    def _prepare_env(self):
        if self.KEYWORD == 'EfficientNetV2':
            self.train_env = Env(bars_count=self.BARS_COUNT,
...
class RL_Train(RL_prepare):
    ...
    def load_pre_train_model_state(self):
        ...
        if checkpoint_path and os.path.isfile(checkpoint_path):
            ...
            print("建立新的儲存點")
            # 用來儲存的位置
            self.saves_path = os.path.join(self.SAVES_PATH, datetime.strftime(datetime.now(), "%Y%m%d-%H%M%S") + '-' + str(self.BARS_COUNT) + 'k-')
