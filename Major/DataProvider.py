...
class DataProvider:
    ...
    def reload_data(
        ...
        if table_name in symbol_name_list:
            ...
            print("創建資料")
            self.SQL.change_db_data(SqlSentense.create_table_name(table_name))
            df = pd.DataFrame()
    ...
    def save_data(
        ...
        if exists == "replace":
            ...
            self.SQL.write_Dateframe(original_df, table_name, exists=exists)

    def get_symbols_history_data(
        self, symbol_type, time_type=None, iflower=True
    ) -> list:
        """
            讀取所有日線資料 用來分析和排序
        Args:
    ...
    def filter_useful_symbol(self, all_symbols: list, tag: str):
        if tag == "VOLUME_TYPE":
            return self.datatransformer.get_volume_top_filter_symobl(
                all_symbols, max_symbols=10
            )
        elif tag == "MTM_TYPE":
    ...
    def get_both_type_targetsymbols(self):
        ...
        """
        all_symbol_name_of_futures = self.Binanceapp.get_targetsymobls(
            symbol_type="FUTURES"
        )
        ...
        data = {
            "Datetime": str(
                datetime.datetime.fromtimestamp(
                    filterdata["t"] / 1000, tz=timezone.utc
                ).replace(tzinfo=None)
