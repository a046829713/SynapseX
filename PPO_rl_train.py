...
from Brain.PPO.lib.environment import Crypto_PortfolioOptimztionEnv
...
class RL_prepare(ABC):
    ...
    def _prepare_envs(self):
        data_path = os.path.join("Brain",'PPO','simulation','train_data.csv')
        data = pd.read_csv(data_path)

        vectors = np.random.rand(10)
        env = Crypto_PortfolioOptimztionEnv(df=data)
        env.reset()
        env.step(vectors)
